...
act_buf		= torch.zeros((num_steps, num_envs) + envs.single_action_space.shape).to(device)
...
for i in range(1, num_iterations + 1):
    ...
    for step in range(num_steps):
        ...
        if "episode" in infos:
            for idx_env in range(num_envs):
                if infos["episode"]["_r"][idx_env]:
                    print(f"step={global_step}, episodic_return={infos['episode']['r'][idx_env]}")
                    writer.add_scalar("charts/episodic_return", infos["episode"]["r"][idx_env], global_step)
                    writer.add_scalar("charts/episodic_length", infos["episode"]["l"][idx_env], global_step)
    ...
    for epoch in range(num_update_epochs):
        ...
        for start in range(0, batch_size, minibatch_size):
            ...
            _, logprob, entropy, newvalue = agent.get_action_and_value(b_obs[mb_inds], b_actions[mb_inds])
